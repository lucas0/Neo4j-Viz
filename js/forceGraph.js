...
function myGraph(el) {
    ...
    this.findLink = function (sourceId,targetId){
      number_of_links = graph["links"].length;
      for (index = 0; index< number_of_links;index++){
        if ((graph["links"][index].startNode == sourceId) && (graph["links"][index].endNode == targetId)){
          return true;
        }
      }
      return false;
    ...
    var update = function () {
        ...
        function isConnected(sourceId, targetId) {
          ...
          for (index = 0; index< number_of_links;index++){
            if (((graph["links"][index].startNode == sourceId) && (graph["links"][index].endNode == targetId)) ||
                ((graph["links"][index].startNode == targetId) && (graph["links"][index].endNode == sourceId))){
              return true;
            }
          }
          return false;
        ...
        var svg = d3.select("#tab1").select("svg")
                    .attr("class","foo")
                    .call(zoom);
        ...
        var pg = container.append("container:g").selectAll("path")
            .data(force.links())
          .enter().append("g");
        ...
        node.enter().append("g")
            ...
            .on('mouseover',function(d){
              node.classed("node-active", function(o) {
                  thisOpacity = isConnected(d.index, o.index) ? true : false;
        ...
        node.append("circle")
            ...
            .on('mouseover', function(d){
                $("#tab2").html(
                  "<strong>ID:</strong> <span style='color:red'>" + d.id +
                  "</br></span></br><strong>Sentence:</strong>"+d.properties.sentence +
                  "</br></span></br><strong>Lemma:</strong>"+d.properties.lemma+
                  "</br></span></br><strong>Sk:</strong>"+d.properties.sk
                );
            })
